...
import { isAdminOrCurrentUser, withMiddleware } from "utils/apiMiddleware";
...
const UserById = withSession(async (request, response) => {
  ...
  if (method === "GET") {
    ...
    if (await isAdminOrCurrentUser(request)) {
  ...
  // and last_login update
    ...
    if (await isAdminOrCurrentUser(request)) {
      ...
      admin
        ...
        .catch((error) => {
          debugError("UPDATING USER, Cant verify user", error);
        });
